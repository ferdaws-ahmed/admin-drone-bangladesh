...
import { Save, FileText, Cpu, HelpCircle, Loader2, CheckCircle2, Package, ChevronDown } from 'lucide-react';
import adminApi from '@/lib/adminApi';

export default function AddDronesMain() {
  const [editId, setEditId] = useState(null);
  const [activeTab, setActiveTab] = useState('basic');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // 🟢 categoryImage ফিল্ড রিমুভ করা হয়েছে
  ...
  const categories = ['Drone', 'Accessories', 'Combo'];
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      try {
        const response = await adminApi.products.getAdminProduct(editId, 'drone');
        ...
        console.error('Failed to load drone product', error);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = editId
        ? await adminApi.products.updateProduct(editId, { ...formData, productType: 'drone' })
        : await adminApi.products.createProduct(formData);
      
      toast.success(response.message || (editId ? 'Drone product updated successfully!' : 'Drone product created successfully!'));
  ...
  return (
    <div className="w-full space-y-5 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Drone Inventory Entry</h1>
        ...
        >
          ...
          <span>{isSubmitting ? 'Saving...' : 'Save Drone Data'}</span>
        </button>
      </div>
      ...
      {/* Only show if subCategory is NOT Accessories or Combo */}
